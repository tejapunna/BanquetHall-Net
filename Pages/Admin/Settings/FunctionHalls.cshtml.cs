...
public class FunctionHallsModel : PageModel
{
    private readonly BanquetHallDbContext _dbContext;

    public FunctionHallsModel(BanquetHallDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public List<FunctionHall> Items { get; set; } = new();

    public async Task OnGetAsync()
    {
        Items = await _dbContext.FunctionHalls.OrderBy(f => f.SortOrder).ToListAsync();
    }

    public async Task<IActionResult> OnPostCreateAsync([FromBody] FunctionHallDto dto)
    {
        if (string.IsNullOrWhiteSpace(dto.Name))
            return new JsonResult(new { success = false, error = "Name is required." });

        var item = new FunctionHall
        {
            Name = dto.Name,
        ...
        _dbContext.FunctionHalls.Add(item);
        await _dbContext.SaveChangesAsync();
        return new JsonResult(new { success = true, data = new { item.Id, item.Name, item.SortOrder, item.IsActive } });
    }

    public async Task<IActionResult> OnPostUpdateAsync([FromBody] FunctionHallDto dto)
    {
        var item = await _dbContext.FunctionHalls.FindAsync(dto.Id);
        if (item == null)
